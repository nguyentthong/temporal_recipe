...
class VQATask(BaseTask):
    ...
    def after_evaluation(self, val_result, split_name, epoch, dataset, **kwargs):
        ...
        elif 'internvid' in self.dataset_name:
            metrics = self._report_metrics_gpt4(result_file=result_file, split=split_name, dataset=dataset, dataset_name=self.dataset_name)
        else:
    ...
    def evaluate_gpt4(self, generation, label):
        ...
        {generation}\n\n
        [System]\n\n
        We would like to request your feedback whether AI assistant answer semantically matches groundtruth.\n\n
    ...
    @dist_utils.main_process
    def _report_metrics_gpt4(self, result_file, split, dataset, dataset_name):
        self.client = OpenAI(
        ...
        for prediction in prediction_list:
            question_id = prediction['question_id']
            pred_answer_list = prediction['answer']
            label = annotation[question_id]
            generation = pred_answer_list[0]
            eval_output = self.evaluate_gpt4(generation, label)
            match_video_list.append(eval_output)
